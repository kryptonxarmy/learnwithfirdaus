...
export default function TrashPage() {
  ...
  const fetchDeletedAttendances = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const res = await fetch("/api/admin/laporan/presensi/trash");
      const data = await res.json();
      
      if (data.success) {
        const teacherAttendances = (data.attendance || []).filter(item => item.type === "teacher");
        setDeletedAttendances(teacherAttendances);
      } else {
  ...
  const handleRestore = async (id) => {
    try {
      const res = await fetch(`/api/admin/laporan/presensi/restore`, {
        method: "PUT",
      ...
      if (data.success) {
        ...
        throw new Error(data.error || "Failed to restore attendance");
      }
    } catch (error) {
  ...
  const handlePermanentDelete = async (id) => {
    try {
      const res = await fetch(`/api/admin/laporan/presensi/permanentDelete`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id }),
      });
      ...
      if (data.success) {
        ...
        throw new Error(data.error || "Failed to delete attendance permanently");
      }
    } catch (error) {
    ...
    return (
      <div className="flex flex-col gap-4 p-4">
        <div className="flex justify-between items-center">
          <h1 className="text-2xl font-bold">Sampah - Data Presensi Guru</h1>
          ...
          <TableBody>
            {deletedAttendances.map((attendance) => (
              <TableRow key={attendance.id}>
                ...
                <TableCell>{attendance.teacher?.name || 'N/A'}</TableCell>
      ...
      <Dialog open={isRestoreDialogOpen} onOpenChange={setIsRestoreDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => handleRestore(selectedAttendanceId)}
              className="bg-primary hover:bg-primary-700 text-white font-semibold rounded-xl px-4"
            >
              Pulihkan
            </Button>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => handlePermanentDelete(selectedAttendanceId)}
              className="bg-red-500 hover:bg-red-600 text-white font-semibold rounded-xl px-4"
            >
              Hapus Permanen
            </Button>
